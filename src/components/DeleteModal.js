...
function DeleteModel({
  ...
  const handleDelete = (e) => {
    let tempTasksList = [...tasksList];
    tempTasksList.splice(dataIndex, 1);
    setTasksList(tempTasksList);
    updateLocal(tempTasksList);
    handleClose();
  };
  return (
    <>
      ...
      <Modal show={show} onHide={handleClose} className="text-light">
        ...
        <Modal.Body>{`Are you sure to delete ( ${tasksList[dataIndex]} ) task`}</Modal.Body>
        <Modal.Footer>
